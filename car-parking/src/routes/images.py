...
async def update_image_description(
    ...
    try:
        ...
        image: Image = await repository_images.update_image_in_db(db=db, image_id=image_id, new_description=body.new_description)
...
@router.get("/transformed_image/{image_id}", 
            response_model=schemas_images.ImageResponse)
async def get_transformed_image(image_id: int, 
                                current_user: User = Depends(service_auth.get_current_user), 
                                db: Session = Depends(get_db)):
    """
